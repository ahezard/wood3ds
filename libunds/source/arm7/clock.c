...
//---------------------------------------------------------------------------------
	...
	time_t time = __mktime(&dstime);
	fifoSendValue32(FIFO_CLOCK,0);
	while(time>0)
	{
		u32 send=FIFO_VALUE32_MASK;
		if(time<FIFO_VALUE32_MASK)
		{
			send=time;
		}
		fifoSendValue32(FIFO_CLOCK,send);
		time-=send;
	}
}
