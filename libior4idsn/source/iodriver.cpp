...
void sddReadBlocks(u32 addr,u32 blockCount,void* buffer)
{
  ...
  if(1==blockCount)
  {
    u32 sdReadSingleBlock[2]={0xab030011,address}; // input read address here
  ...
  {
    u32 lastSector=ioSize()-1;
    ALIGN(4) u8 response[4];
    u32 sdReadMultiplyBlock[2]={0xab040012,address}; // input read address here
...
void sddGetSDInfo(u8 info[8])
{
  ...
  {
    ALIGN(4) u8 response[4];
    u32 cmd[2]={0xab010007,0x00000000};
    sendSDCommandR1(cmd,response);
    dbg_printf("cmd 07 response %08x\n",*(u32*)response);
  }
  ...
  {
    ALIGN(4) u8 response[16];
    u32 cmd[2]={0xab01000A,0x00000000|(SDCardAddr<<16)};
    sendSDCommandR2(cmd,response);
